...
public class GPMDaoImpl implements GPMDao {
	...
	public void GPMLogin() throws SQLException {
		try {
			...
			ResultSet rs = statement
					.executeQuery("select * from gpm where email='" + email + "' AND password ='" + password + "' ");
			if (rs.next()) {
	...
	public void createEmployee() throws SQLException {
		try {
			...
			statement.execute("insert into Employee (ename,email,password,area,pincode,age,gid)" + "values('" + name
					+ "','" + email + "','" + password + "','" + area + "','" + pincode + "','" + age + "','" + id
					+ "')");
